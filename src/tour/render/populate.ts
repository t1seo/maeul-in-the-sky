...
export function populateVillage(model: TourModel, resources: GeometryResources) {
  ...
  for (const [index, placement] of model.placements.entries()) {
    ...
    for (const part of asset.recipe.parts) {
      ...
      batches.add(part, asset.recipe.key, position, asset.scale, source.id, glow);
    }
  ...
  root.add(shadows, addScenery(model, batches, resources), batches.finish());
  return { root, colliders, identities: batches.identities, glows: batches.glows };
}
