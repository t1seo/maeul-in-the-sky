...
- 일반 에셋 ID 202개: 클래식 189개와 한국형 13개, 계절형 68개와 전 계절형 134개
- Rare, Epic, Legendary 등급으로 발견하는 Epic Wonders 30종
- 기존 223개 ID의 그림을 개편하고 한국 시골 에셋 9개를 추가해 총 232종
...
다크와 라이트는 한 번 준비한 장면을 색칠하므로 지형, 일반 에셋, Wonder의 기하가 일치합니다. 새 장면은 `layoutVersion: 2`를 내보내며 정규화한 사용자명, 선택적 `layoutSeed`, 절대 날짜를 사용합니다. 이전 버전 1 장면도 렌더링할 수 있지만, 다시 생성하면 버전 2가 적용되어 배치가 달라질 수 있습니다. 고정 정규화와 주변 맥락이 같으면 이동한 기간의 내부 겹침 날짜는 지형과 일반 배치를 유지하지만 화면 위치는 바뀝니다. 상대 P90은 높이를 바꿀 수 있고, 이웃과 전역 Wonder 예산은 선택을 바꿀 수 있습니다. 설정, 범위, 배치 버전이 다르면 같은 픽셀을 보장하지 않습니다.

계절은 실제 날짜와 반구를 따릅니다. 봄의 꽃과 새잎, 여름의 풍성한 녹음, 가을의 단풍과 수확물, 겨울의 눈과 앙상한 가지를 실루엣과 재질 색상으로 구분합니다. 한국형은 기존 `hanok`, `pavilion`, `stoneWall`, `onggi`에 `choga`, `jangseung`, `sotdae`, `riceTerrace`, `koreanWatermill`, `hanokGate`, `kimchiGarden`, `stoneBridge`, `hanokEstate`를 더합니다. 높은 단계의 일반 보상도 한국 시골 건축을 사용하며, 조건에 맞는 이웃 건물 사이에는 길이 생깁니다. 기여 수와 Wonder 조건은 바뀌지 않습니다.
...
[인터랙티브 카탈로그](https://t1seo.github.io/maeul-in-the-sky/catalog/)는 미니어처/픽셀 선택과 한국형 13개 전체를 제공합니다. 일반 에셋 **202개 = 클래식 189개 + 한국형 13개**, Wonder **30개 = Rare 14개 + Epic 10개 + Legendary 6개**를 따로 집계하며 합계는 **232개**입니다. 일반 에셋 중 68개는 계절형, 134개는 전 계절형입니다. 변형·그림체·배치 인스턴스는 ID 수를 늘리지 않습니다. [온라인 Wonder 도감](https://t1seo.github.io/maeul-in-the-sky/#wonders)에서는 실제 발견 상태와 조건을 볼 수 있습니다.
