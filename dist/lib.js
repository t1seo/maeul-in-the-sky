...
var archiveSchema = z3.object({
  schemaVersion: z3.literal(1),
  kind: z3.literal("maeul-archive"),
  snapshots: archiveSnapshotsSchema,
  comparison: z3.object({
...
var countSchema = z4.number().finite().int().nonnegative();
var calendarSchema = z4.object({
  totalContributions: countSchema,
  weeks: z4.array(
    z4.object({
      contributionDays: z4.array(
        z4.object({
          date: z4.iso.date(),
          contributionCount: countSchema,
          contributionLevel: z4.enum([
            "NONE",
            "FIRST_QUARTILE",
            "SECOND_QUARTILE",
            "THIRD_QUARTILE",
...
var graphQLErrorSchema = z4.object({
  type: z4.string().optional(),
  message: z4.string(),
  extensions: z4.object({ code: z4.string().optional() }).optional()
});
var envelopeSchema = z4.object({
  data: z4.object({
    user: z4.object({ contributionsCollection: z4.object({ contributionCalendar: calendarSchema }) }).nullable()
  }).nullish(),
  errors: z4.array(graphQLErrorSchema).optional()
});
var errorBodySchema = z4.object({
...
async function renderPng(svg, options) {
  const scale = parseBoundary(z5.number().int().min(1).max(4), options.scale ?? 2, "scale");
  const mode = parseBoundary(z5.enum(["dark", "light"]), options.mode, "mode");
...
function optionalNumber(value, field) {
  ...
  const parsed = z6.coerce.number().finite().safeParse(value);
...
function parseOutputOptions(request) {
  const format = parseBoundary(z6.enum(["svg", "png", "both"]), request.format || "svg", "format");
  const scale = parseBoundary(
    z6.number().int().min(1).max(4),
...
function parseGenerationYears(value) {
  ...
  const years = parseBoundary(z6.array(yearSchema).min(2).max(5), values, "years");
...
init_schema();
var requestSchema = z7.strictObject({
  username: usernameSchema,
  year: z7.number().int().min(2008).max(9999).optional(),
