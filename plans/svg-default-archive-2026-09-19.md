...
## Added scope: terrain, currents and sky waterfalls

The user added ground/water quality improvements before publication. The release now also adds date-stable ground material variants and cliff relief, clearer directional river currents, and a maximum of four sky waterfalls with fading water and mist. This remains miniature SVG rendering only; scenes, contribution heights, Classic/pixel, and archived 3D keep their existing data contracts.

Metis review `.orca/svg-default/water-metis.md` identified outlet priority, partial calendars, animation allocation and render-only bounds as acceptance requirements. Root owns water topology/shapes/motion, render fitting and E2E tests; the ground worker owns `surface-block.ts` and its unit tests. River terminals must be selected before optional coastal spillways; internal missing dates never create a mouth. Waterfalls and currents share the original 15/full and 4/subtle water target budget, within the total 50. Reduced/off retain complete still water geometry. Rendering unions waterfall/mist bounds without editing prepared scene bounds.

Before publication: run topology/season/data preservation and global-budget tests; verify real water-only motion, reduced/off, banner/card fit in desktop/mobile browsers; inspect Day/Night and a real profile in Orca; review and update only the SVG static goldens, rebuild bundles and regenerate current preview/preset SVGs. Keep benchmark thresholds, archived world goldens and release version unchanged. Repeat the five release reviews for the added artwork, then require final CI, Pages deployment and the actual SVG-only profile workflow.
